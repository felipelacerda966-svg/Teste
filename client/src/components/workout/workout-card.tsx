...
export default function WorkoutCard({ userWorkout }: WorkoutCardProps) {
  ...
  const getStatusBadge = () => {
    if (userWorkout.completedDate) {
      return <Badge className="bg-accent text-white">Concluído</Badge>;
    }
    if (progressPercentage > 0) {
      return <Badge className="bg-yellow-500 text-white">Em Progresso</Badge>;
    }
    return <Badge className="bg-accent text-white">Agendado</Badge>;
  };

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'chest':
        return <Dumbbell className="text-primary h-5 w-5" />;
      case 'cardio':
        return <div className="text-accent h-5 w-5 flex items-center justify-center">🏃</div>;
      default:
        return <Dumbbell className="text-primary h-5 w-5" />;
    }
  };
      ...
      <CardContent className="p-6">
        ...
        {userWorkout.completedDate ? (
          ...
          <Button 
            ...
            className="w-full bg-primary text-white hover:bg-red-600"
          >
